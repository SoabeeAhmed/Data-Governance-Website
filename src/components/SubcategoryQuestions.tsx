...
//     if (allAnswered && !hasTriggeredCompletion && !isAlreadyCompleted) {
...
const SubcategoryQuestions: React.FC<SubcategoryQuestionsProps> = ({
  ...
  const [currentAnswers, setCurrentAnswers] = useState<{
    [index: number]: { score: number; suggestion?: string };
  }>({});
  const [averageScore, setAverageScore] = useState<number>(0);
  const [allQuestionsAnswered, setAllQuestionsAnswered] = useState<boolean>(false);
  const [hasTriggeredCompletion, setHasTriggeredCompletion] = useState<boolean>(false);
  ...
  useEffect(() => {
    const totalScore = Object.values(currentAnswers).reduce((acc, ans) => acc + (ans.score ?? 0), 0);
    const answeredCount = Object.values(currentAnswers).filter(ans => ans.score !== undefined).length;
    const score = answeredCount > 0 ? totalScore / answeredCount : 0;
    setAverageScore(score);
  ...
  const updateLocalStorage = (updatedAnswers: typeof currentAnswers) => {
    const updatedAllAnswers = {
      ...allAnswers,
      [activeCategory]: {
        ...allAnswers[activeCategory],
        [activeSubcategory]: updatedAnswers,
      },
    };
    localStorage.setItem("dataQualityAssessmentAnswers", JSON.stringify(updatedAllAnswers));
  ...
  const handleSuggestionChange = (index: number, suggestion: string) => {
    const updated = {
      ...currentAnswers,
      [index]: {
        ...currentAnswers[index],
  ...
  return (
    <div className="p-4 bg-white shadow rounded">
      ...
      <div className="flex-row-container">
        ...
        <div className="alignment-container mt-6 mb-6">
          <h3 className="text-lg font-semibold score">
            Questions Attempted: {Object.keys(currentAnswers).filter(k => currentAnswers[+k]?.score !== undefined).length} / {questions.length}
            <br />
            Average Score: {averageScore.toFixed(1)} / 5
            {allQuestionsAnswered && !hasTriggeredCompletion && !isAlreadyCompleted && (
              <div className="mt-2 text-green-600 font-bold">
      ...
      {questions.length > 0 ? (
        questions.map((q, idx) => (
          <QuestionItem
            ...
            question={q.text}
            options={q.options}
            value={currentAnswers[idx]?.score}
            suggestion={currentAnswers[idx]?.suggestion || ""}
            onChange={handleAnswerChange}
